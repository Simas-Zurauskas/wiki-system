...
`recheck`, and `notion sync` never touch it — they only suggest running this one.
...
**AI agents: start at** <link to wiki/AI/index.md> — the agent-optimized track
...
`wiki-acme/AI/index.md`. `CLAUDE.md` sits beside that folder in the workspace, so the
links are simple relative paths.

The `AI` track (`wiki/AI/index.md`) is **always on**, so the "AI agents: start at" line
is always present — lead the Documentation section with it. `CLAUDE.md` only *points to*
the `AI` track — it is the lean signpost an agent reads first; the agent-audience depth
lives in `wiki/AI/`, never restated here.
