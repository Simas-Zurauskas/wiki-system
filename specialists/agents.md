Your task is to write the **agents track** of this project's wiki: documentation optimized
...
plan → execute). You plan the whole agents track yourself. (`CLAUDE.md` is never written here
...
1. Read `wiki/.internal/plan.yaml` in full before anything else. It is the authoritative
   plan. Do not re-plan the agents track's shape. If a field's meaning is unclear, consult
   `../spec/plan-schema.md`.
...
After you finish, a verifier sub-agent (`./verifier.md`, **mode `agents`**) checks your
page's claims against your `scope_files`. The `agents` mode allows code references (unlike
...
## WHAT THE AGENTS TRACK IS (AND IS NOT)
...
| **`wiki/AGENTS` (you)** | an agent doing a *specific task* | **retrieved in fragments, on demand** | **the invariants an agent must not violate, the contracts of agent-facing surfaces, and the runbooks for common changes** |
...
5. **No near-duplicates.** Do not restate paragraphs that live in the technical track or in
   another `AGENTS/` page — link instead. Conflicting/overlapping passages hurt agents more
   than unrelated ones.
...
Bootstrap / Growth / Maintenance, same as the other specialists. If a `wiki/AGENTS`
already exists, read it before planning; audit for drift, stale anchors, and duplication
against the technical track.

### 3. PLAN

Plan the agents track structure (see OUTPUT STRUCTURE). Decide which contracts, runbooks, map
...
(`../claude-md.md`). The generated `CLAUDE.md` will *point* agents at `wiki/AGENTS`;
you never write it.
...
All agents-track output lives under `wiki/AGENTS`. The track is **standalone-complete**:
...
for deep narrative — but keep the agents track's own pages, lean and agent-oriented, in place.
...
- **Tiny repo:** collapse to a few flat files — `AGENTS/index.md` + `AGENTS/invariants.md` + one
...
### wiki/AGENTS/index.md (the machine index)
...
- Code references are allowed, but every symbol/path/type/command/anchor must resolve in
  source.
- Every page must be reachable from `AGENTS/index.md` (directly or via a folder `index.md`).
