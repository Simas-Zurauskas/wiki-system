...
**Generated header.** The one-line generated-header at the top of every page
...
is boilerplate the verifier IGNORES exactly like an `AUTOREGEN_SKIP` block: it is
not extracted as a claim, not flagged as unanchored, and not counted in
`total_claims`.
...
- The one-line generated-header at the top of the page — boilerplate, ignored
  exactly like an `AUTOREGEN_SKIP` block (never counted in `total_claims`)
